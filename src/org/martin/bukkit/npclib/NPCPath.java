...
public class NPCPath {
	...
	Material[] nonSolid = { Material.AIR, Material.SAPLING,
			...
			Material.REDSTONE_WIRE, Material.CROPS, Material.SIGN_POST,
			Material.WALL_SIGN, Material.LEVER, Material.STONE_BUTTON,
	...
	private void scoreBlock(Node node, Node parent) {
		int diagonal = ((node.xPos != parent.xPos && node.zPos != parent.zPos)
				|| (node.xPos != parent.xPos && node.yPos != parent.yPos) || (node.yPos != parent.yPos && node.zPos != parent.zPos)) ? 14
				: 10;
